...
int main() {
    ...
    while (!WindowShouldClose()) {
        ...
        if (menuState == MAIN_MENU) {
            ...
            DrawText("S:Split | P:Pause | R:Restart | F5:Save | L:Leaderboard", SCREEN_WIDTH - 320, SCREEN_HEIGHT - 30, 14, Fade(WHITE, 0.4f));
        }
            ...
            if (showLeaderboard) {
                ...
                for (size_t i = 0; i < leaderboard.size(); i++) {
                    int y = 140 + i * 35;
                    Color c = (i == 0) ? COLOR_GOLD : (i == 1) ? {192,192,192,255} : (i == 2) ? {205,127,50,255} : WHITE;
